...
@pytest.mark.anyio
async def test_intake_messages_never_promote_to_gate1(tmp_path):
    """
    process_intake_message always stays in 'intake' status.
    gate1 is only set by intake_graph (POST /run with Gemini), never from messages alone.
    """
    caso_id = "intake_no_gate_test"
    store = _make_store(tmp_path, caso_id)
    _create_initial_state(store, caso_id)
    state = store.load_latest_state()
    ...
    assert state.status == "intake"
    assert len(state.intake_history) == 3
